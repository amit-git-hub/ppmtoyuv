...
public:
	...
	if(fin.is_open())
	{
		...
		for(auto j=0;j<width;j++)
		{
		    int32_t y;
		    int32_t r, g, b;
		    const int32_t rCoeff = 306, gCoeff = 601, bCoeff = 117;
		    const int32_t cbCoeff = 579, crCoeff = 730;
	    ...
	    fout.write(reinterpret_cast<char*>(yBuf.get()), width*height);
	    fout.write(reinterpret_cast<char*>(cbBuf.get()), width*height);
	    fout.write(reinterpret_cast<char*>(crBuf.get()), width*height);
